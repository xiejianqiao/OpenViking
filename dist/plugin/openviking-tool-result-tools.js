...
export function registerOpenVikingToolResultTools({ registerTool, getClient, resolvePluginSessionRouting, isBypassedSession, makeBypassedToolResult, logger, }) {
    registerTool((ctx) => ({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                const result = await client.readToolResult(session.ovSessionId, parsed.toolResultId, { offset, limit, includeMetadata: true });
    ...
    registerTool((ctx) => ({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                const client = await getClient();
                const result = await client.searchToolResult(session.ovSessionId, parsed.toolResultId, query, { limit, contextChars });
    ...
    registerTool((ctx) => ({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                const client = await getClient();
                const result = await client.listToolResults(session.ovSessionId, { toolName, limit });
