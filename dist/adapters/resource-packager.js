...
export function isRemoteResourceSource(source) {
    return REMOTE_RESOURCE_PREFIXES.some((prefix) => source.startsWith(prefix));
}
export async function cleanupUploadTempPath(path) {
    if (!path) {
        return;
    }
...
export async function zipDirectoryForUpload(dirPath) {
...
async function prepareLocalUploadSource(path, includeSourceName = false) {
...
export const defaultResourcePackager = {
    async prepareResourceSource(pathOrUrl) {
        if (isRemoteResourceSource(pathOrUrl)) {
            return { kind: "remote", path: pathOrUrl };
        }
        return prepareLocalUploadSource(pathOrUrl, true);
    },
    async prepareLocalUploadSource(path) {
        return prepareLocalUploadSource(path, false);
    },
    async createTempUploadBody(uploadPath) {
        const fileBytes = await readFile(uploadPath);
        const form = new FormData();
        form.append("file", new Blob([toBlobPart(fileBytes)], { type: "application/octet-stream" }), basename(uploadPath));
        return form;
    },
