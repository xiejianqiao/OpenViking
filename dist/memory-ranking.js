...
function lexicalOverlapBoost(tokens, text, maxBoost = DEFAULT_RANKING_WEIGHTS.lexicalOverlapMax) {
...
function inferResourceType(item) {
    if (item.uri.startsWith("viking://resources"))
        return "resource";
    if (item.uri.startsWith("viking://session/"))
        return "session";
    if (item.uri.startsWith("viking://user/skills") || item.uri.startsWith("viking://skills"))
        return "agent";
    if (item.uri.startsWith("viking://user/"))
        return "user";
    return undefined;
}
function rankForInjection(item, query, options) {
...
export function pickMemoriesForInjection(items, limit, queryText, scoreThreshold = 0, rankingOptions) {
